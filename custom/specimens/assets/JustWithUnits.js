...
var JustWithUnits = (function () {
  ...
  function JustWithUnits ( querySelection, userConfig ) {
    ...
    console.log( "JUSTWITHUNITS querySelection", querySelection );
    ...
    this.wsMinPercentage        = 65 
    this.wsOptPercentage        = 75
    this.wsMaxPercentage        = 90
    ...
    if( userConfig ){
      ...
      if( userConfig.wsMinPercentage >= 0 && userConfig.wsMinPercentage <= 200 ) this.wsMinPercentage = userConfig.wsMinPercentage; 
      if( userConfig.wsOptPercentage >= 0 && userConfig.wsOptPercentage <= 200 ) this.wsOptPercentage = userConfig.wsOptPercentage; 
      if( userConfig.wsMaxPercentage >= 0 && userConfig.wsMaxPercentage <= 200 ) this.wsMaxPercentage = userConfig.wsMaxPercentage; 
    }

    console.log( "JUSTWITHUNITS config", this.overshootPercentage, this.alignment, this.algorithm, this.scaling, this.hyphenation );
    ...
    theStyle.type = 'text/css'; //assets/OHR3.ttf
    theStyle.innerHTML = '.cssClass { color: #f00; } @font-face { font-family: TESTFONT; src: url(../../fonts/ttf/KierkegaardSerifText-Regular.ttf); } body { font-family: TESTFONT; font-variation-settings: "wght" 300, "xtra" 300, "wdth" 100; } .line { display: flex; justify-content: space-between; position: relative; } .align-justify .line { justify-content: space-between; text-indent: 0; } .align-center .line { justify-content: center; } .align-left .line { justify-content: flex-start; } .align-right .line { justify-content: flex-end; } .align-justify .line:last-child, .align-left .line:last-child, .align-right .line:last-child { justify-content: flex-start; __background-color: white; } .align-right .line:last-child { justify-content: flex-end; } .line:last-child { justify-content: flex-start; } .word { __background-color: white; white-space: nowrap; text-indent: 0; } .space { __background-color: white; min-width: 0; text-indent: 0; } .hyphen { __background-color: white; text-indent: 0; } .pone { font-variation-settings: "wght" 300, "xtra" 300, "wdth" 110; color: var(--unitizedColor); }';
    document.getElementsByTagName('head')[0].appendChild(theStyle);
    
    var hy = new Hypher( language );

    const theElementsList = document.querySelectorAll(querySelection)
    for (let i = 0; i < theElementsList.length; i++) {
      debug ? console.log( "Element", i ) : 0 ;
      ...
      debug ? console.log( "font size in px", window.getComputedStyle(theElement).getPropertyValue("font-size") ) : 0 ;
      var theMeasure = theBoundingBox['width']

      // textIndent
      let spacer = " "; // e.g. sechstelgeviert
      let spacerWidth = measureText( spacer );
      let numSpacers = Math.round( parseInt(window.getComputedStyle(theElement).getPropertyValue("text-indent"),10) / spacerWidth );
      debug ? console.log("spacer", spacerWidth, numSpacers) : 0;
      var theContent = spacer.repeat(numSpacers) + theElement.innerHTML.replace(/\s\s+/g, ' ') // removes whitespace sequences
      
      // Hyphenation zur Zeit nur in englisch
      //console.log ( hy.hyphenateText( 'We encourage students to take an active role by asking questions.' ).replaceAll( "\u00AD", "|"));
      if( this.hyphenation ) theContent = hy.hyphenateText( theContent ).replaceAll( "\u00AD", "|");
      debug ? console.log(theContent) : 0 ;

      let saveSpace = theElement.innerHTML
      theElement.innerHTML = ''
      update(theContent, theMeasure, this.overshootPercentage, this.alignment, this.algorithm, this.scaling, this.hyphenation, this.wsMinPercentage, this.wsOptPercentage, this.wsMaxPercentage )
      if( theElement.innerHTML == '' ){
        theElement.innerHTML = saveSpace;
        debug ? console.warn( "Something went wrong.") : 0;
      } else {
        debug ? console.info( "Element sucessfully justified.") : 0;
      }
    }

    function measureText (str) {
      if (str === ' ') { str = '\u00A0' }
      theRuler.textContent = str
      return theRuler.getClientRects()[0].width ;
    }
    ...
    function typeset ( nodes, breaks, measure, algorithm, overshootPercentage, scaling ) {
      ...
      lines.forEach(function (line, lineIndex, lineArray) {
        ...
        debug ? console.log( "line", lineIndex ) : 0;
    ...
    function update (text, theMeasure, theOvershootPercentage, theAlignment, theAlgorithm, theScaling, theHyphenation, theWsMinPercentage, theWsOptPercentage, theWsMaxPercentage ) {
      ...
      let wsMinPercentage = theWsMinPercentage
      let wsOptPercentage = theWsOptPercentage
      let wsMaxPercentage = theWsMaxPercentage

      let totalLeftOverSpace = 0

      if (algorithm === 'knuth-and-plass') {
        let { nodes, breaks } = kap( text, measureText, alignment, measure, hyphenation, wsMinPercentage, wsOptPercentage, wsMaxPercentage, overshootPercentage )
        totalLeftOverSpace = typeset( nodes, breaks, measure, algorithm, overshootPercentage, scaling )
      } else {
        let { nodes, breaks } = greedy( text, measureText, alignment, measure, hyphenation, wsMinPercentage, wsOptPercentage, wsMaxPercentage, overshootPercentage )
        totalLeftOverSpace = typeset( nodes, breaks, measure, algorithm, overshootPercentage, scaling )
      }
    }
  }
...
// break not only at vertical bar but also by already existing hyphen 

function greedy(text, measureText, alignment, measure, hyphenation, wsMinPercentage, wsOptPercentage, wsMaxPercentage, overshootPercentage ) {

  ///var wsOptPercentage     = 75; 
  ///var overshootPercentage = 90;
...
function kap(text, measureText, alignment, measure, hyphenation, wsMinPercentage, wsOptPercentage, wsMaxPercentage, overshootPercentage ) {

  ///var wsMinPercentage     = 65; 
  ///var wsOptPercentage     = 75; 
  ///var wsMaxPercentage     = 90; 
  ///var overshootPercentage = 90;
  ...
  let spaceWidth = measureText('\u00A0') * wsOptPercentage / 100 ;  /////2025-05-03

  let nodes = [];

    // 2025-05-03 console.log( "spaceWidth:", spaceWidth );
  ...
  text.split(/(\s|-|\|)/).forEach(function (fragment) {
    if (alignment === 'align-justify') {
      let wsMin = ( spaceWidth * wsMinPercentage / 100 );
      let wsOpt = ( spaceWidth * wsOptPercentage / 100 );
      let wsMax = ( spaceWidth * wsMaxPercentage / 100 );
      let realHyphenWidth = hyphenWidth - (hyphenWidth * overshootPercentage / 100); 
      if (fragment === ' ') {
        nodes.push(linebreak.glue( wsOpt, wsMax, wsMin ));  
      } else if (fragment === '|') {
        nodes.push(linebreak.penalty( realHyphenWidth, 100, 1 ));
      } else {
  ...
  if (!breaks.length) { // 2025-05-01
    breaks = linebreak(nodes, [measure], { tolerance: 70, demerits });
  }
...
var linebreak = function (nodes, lines, settings = {
  ...
  if (activeNodes.size !== 0) {
    ...
    //console.warn('Overfull paragraph.', activeNodes.size);  /// 2025-04-08
        //console.warn('Overfull paragraph.'); // 2025-05-01
